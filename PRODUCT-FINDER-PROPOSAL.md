...
5. Customize the questions (up to one per finder attribute — five today, see §13).
6. Set each question as hard filter or soft preference; fallback-relaxation order (§5d) isn't a separate step — it's simply the order the hard-filter questions are listed in.
...
- ✅ **Done.** Question customization (§6 step 5–6) shipped as a 4-phase follow-on to the original build order: (1) a `QuestionSetResolver` data/resolution layer — a category's saved question set overrides `TentsTemplate`'s defaults, with relaxation order derived from the hard-type questions' own order rather than declared separately; (2) real-value auto-discovery for select-type answer choices, so a merchant never has to type/guess a value that has to exactly match what's stored; (3) the admin mini rule-builder UI itself — one row per finder attribute (question text, short label, hard/soft, comparator, weight, choices-vs-toggle), pre-filled from the template as an editable starting point; (4) this note. Scope stayed deliberately narrower than a full editor: the finder-attribute set itself (capacity/packed_weight/season_rating/use_type/price) is still fixed — customizing *how* they're asked about, not inventing new ones. That's still a real boundary, and a candidate for the premium "more question types" tier (§12) if a merchant ever needs it.
